...
function hp_heqi_check(n,l,w)
 ...
 if job.name=='songmoya' then
  ...
  if yptjob.name1~='无威胁' and yptjob.name2~='无威胁' then      
   ...
   if heqi > 500 and score.party=="桃花岛" then
      exe('perform feiying;perform tan;perform shentong')
   end
...
function job_exp_trigger()
	...
	--你因为自己拾「信」不昧，获得了四百零二点经验，一百一十五点潜能。
	--你获得了四百四十七点经验，一百五十八点潜能。
	create_trigger_t('job_exp9',"^(> )*你因为自己拾「信」不昧，获得了(\\D*)点经验，(\\D*)点潜能。$",'','job_exp_lostletter')
	create_trigger_t('job_exp91',"^(> )*你获得了(\\D*)点经验，(\\D*)点潜能。$",'','job_exp_lostletter')
	...
	SetTriggerOption("job_exp91","group","job_exp")
...
function job_exp_lostletter(n,l,w)
    messageShow('LostLetter奖励：经验:【'..w[2]..'】点，潜能:【'..w[3]..'】点！')
end
