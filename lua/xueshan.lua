...
function xueshan_trigger()
    ...
    create_trigger_t('xueshan_sa2','^(> )*hmm，你似乎在利用BUG！','','xueshansa_look')
...
function xueshan_judge()
    ...
    if job.party and job.party=='姑苏慕容' and job.weapon=='sword' then
       messageShow('雪山任务：遇到姑苏慕容，装备武器：长剑。使用慕容剑法专用PFM！')
	   exe('pfmmrjf')
    end
    if job.party and job.party=='姑苏慕容' and job.weapon=='blade' then
       messageShow('雪山任务：遇到姑苏慕容，装备武器：钢刀。使用妙属性PFM！')
	   exe('pfmmiao')
    end
    if job.party and job.party=='明教' and job.weapon=='dagger' then
       messageShow('雪山任务：遇到明教，装备武器：一块铁令。使用圣火令法专用武器PFM！')
	   exe('pfmshlf')
    elseif job.party and job.party=='明教' then
       messageShow('雪山任务：遇到明教，武功：寒冰绵掌。使用刚属性PFM！')
	   exe('pfmgang')
    end    
    if job.party and job.party=='少林' and job.weapon=='whip' then
       messageShow('雪山任务：遇到少林，装备武器：长鞭。武功：日月鞭法。使用险属性PFM！')
	   exe('pfmxian')
    elseif job.party and job.party=='少林' then
       messageShow('雪山任务：遇到少林，武功；达摩剑OR一指禅。使用妙属性PFM！')
	   exe('pfmmiao')
    end
    ...
    if job.party and job.party=='华山' and job.weapon=='sword' then
       messageShow('雪山任务：遇到华山。装备武器：长剑。武功：华山剑法。使用妙属性PFM！')
	   exe('pfmmiao')
       elseif job.party and job.party=='华山' and job.weapon=='blade' then
      messageShow('雪山任务：遇到华山。装备武器：钢刀。武功：反两仪刀。使用险属性PFM！')
	   exe('pfmxian')
    end
    if job.party and job.party=='昆仑派' and job.weapon=='sword' then
       messageShow('雪山任务：遇到昆仑派。武功：剑法。使用柔属性PFM！！')
	   exe('pfmrou')
    end
    if job.party and job.party=='昆仑派' and job.weapon=='unarmed' then
       messageShow('雪山任务：遇到昆仑派。武功：昆仑叠掌or穿云腿法。使用刚属性PFM！！')
	   exe('pfmgang')
    end
