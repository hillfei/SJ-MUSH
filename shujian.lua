...
function checkPrepareOver()
  ...
  if not job.last or job.last=="songxin" or needdolost==1 or job.last=="tdh" or job.last=="songmoya" 
			or job.zuhe["hqgzc"] or job.zuhe["huashan"] or job.zuhe["wudang"] then
    return check_xuexi()
  else
     return check_job()
  end
...
function checkBagsId(n,l,w)
   ...
   if string.len(l_name)==6 and (string.sub(l_name,5,6)=="药" or string.sub(l_name,5,6)=="丸") and (not drugPoison[l_name] and not drugBuy[l_name]) then
...
function get_thd_fenglu()
    ...
    messageShow('桃花岛领俸禄:我'.. score.age ..'岁已经到归云庄领过俸禄了')
    return check_heal()
end
...
function check_pot(p_cmd)
 if hp.exp<5000000 then
   ...
   if score.party=="普通百姓" and hp.pot>=l_pot and score.gold and skills["literate"] and score.gold>1000 and skills["literate"].lvl<hp.pot_max-100 then
      return literate()
   end
