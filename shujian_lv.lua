...
function xuexi()
	...
	if hp.exp<150000 then
	   master.times=10
	    ...
	    -- ain usepot
	   master.times=math.modf(hp.jingxue_max/60)
...
function literate()
	...
	if hp.exp<151000 then
	    ...
	    master.times=math.modf(hp.jingxue_max/100)
	    if master.times>50 then master.times=50 end
	end
...
function check_pot(p_cmd)
 if hp.exp<5000000 then
   ...
   if score.party~="普通百姓" and hp.pot>=l_pot and flag.autoxuexi and flag.autoxuexi==1 then
      if score.gold and skills["literate"] and score.gold>900 and skills["literate"].lvl<hp.pot_max-100 then
         return literate() 
      end
